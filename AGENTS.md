...
## 라이선스 주기 재검증
- 앱 실행 후에도 시작 시 1회 검증으로 끝내지 않고, `60분(3600초)`마다 라이선스를 재검증한다.
- 재검증 실패(미등록/만료/검증 오류 포함) 시 실행 중 사용자라도 즉시 강제 차단한다.
- 강제 차단 시 자동화 작업과 관련 타이머를 중지하고, 사용자에게 차단 사유를 안내한 뒤 프로그램을 종료한다.
- 위 주기 재검증/강제 차단 동작은 `.py`와 `.exe` 배포본에서 동일하게 유지한다.
